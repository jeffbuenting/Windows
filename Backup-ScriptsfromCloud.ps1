...
$Cred = New-Object -TypeName System.Management.Automation.PSCredential -ArgumentList 'xxxxxx',(Get-Content -Path 'F:\OneDrive - StratusLIVE, LLC\Scripts\cred.txt' | ConvertTo-SecureString ) 
...
get-childitem -Path 'j:' -recurse | where PSIsContainer -eq $False | foreach {
    write-host "     F:\OneDrive - StratusLIVE, LLC\Scripts$(($_.FullName).substring(2))" -ForegroundColor Cyan

    if ( Test-path -Path "F:\OneDrive - StratusLIVE, LLC\Scripts$(($_.FullName).substring(2))" ) {
            $File =  Get-ChildItem -Path "F:\OneDrive - StratusLIVE, LLC\Scripts$(($_.FullName).substring(2))"
            if ( $File.LastWriteTime -le $_.LastWriteTime ) {
                    Write-Host "          Copying FROM Cloud" -ForegroundColor Cyan
                    Copy-Item -Path $_.FullName -Destination "F:\OneDrive - StratusLIVE, LLC\Scripts$(($_.FullName).substring(2))" -Force
                }
                else {
                    Write-Host "          Copying TO Cloud" -ForegroundColor Magenta
                    Copy-Item -Path "F:\OneDrive - StratusLIVE, LLC\Scripts$(($_.FullName).substring(2))" -Destination $_.FullName -Force
            }
        }
        Else {
           
            if ( -not (Test-Path -Path "F:\OneDrive - StratusLIVE, LLC\scripts$(($_.DirectoryName).substring(2))") ) {
                MD "F:\OneDrive - StratusLIVE, LLC\scripts$(($_.DirectoryName).substring(2))"
            }
